...
<div id="content">
    <div class="card">
        ...
        <form method="get" action="">
            <input type="text" name="search" placeholder="Search by Order ID or Customer ID" value="<?php echo isset($_GET['search']) ? $_GET['search'] : ''; ?>">
    ...
    if (!empty($search)) {
        ...
        $query = "SELECT * FROM orders WHERE OrderID LIKE '%$search%' OR CustomerID LIKE '%$search%' OR ShippingAddress LIKE '%$search%' OR BillingAddress LIKE '%$search%' LIMIT $offset, $ordersPerPage";
        $countQuery = "SELECT COUNT(*) AS total FROM orders WHERE OrderID LIKE '%$search%' OR CustomerID LIKE '%$search%' OR ShippingAddress LIKE '%$search%' OR
         BillingAddress LIKE '%$search%'";
    } else {
